...
svName = input("Server name (BMP, ROS, PP1, MDW, ETC)\n")


imagesPath = infoSvs[svName]['local']

wimPath = str(imagesPath) + '\\NewWim11\\Win1122H2WDS07_Test.wim'

testRelease = 'Set-WdsBootImage -ImageName "03 - ' + str(get_month_one_month_ago()) + ' Accenture Global Image" -ImagePath ' + str(wimPath) + ' -DisplayOrder 3'
print(testRelease)
subprocess.run(['powershell', '-Command', testRelease], capture_output=True, text=True)
time.sleep(10)
...
#et-WdsBootImage -ImageName "My Custom Boot Image" -ImagePath "C:\Path\To\Your\Customized\WinPE.wim"
