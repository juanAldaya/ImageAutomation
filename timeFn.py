...
def getDrivesLetters ():
    devices = captureOutput()
    drivesList = []
    i = 0
    for char in devices:
        if char == ':':
            driveLetter = devices[i-1]
            drivesList.append(driveLetter)
        i = i + 1
    return drivesList

def getDrivesStrings(driveLetter):
    driveString = win32api.GetVolumeInformation(driveLetter + ":\\")[0]
    return driveString

def wkLetter (): #wkstation drive disk 
    drivesList = getDrivesLetters()
    for drives in drivesList:
          if getDrivesStrings(drives) == 'WorkstationImage':
            letter = drives
    return letter
